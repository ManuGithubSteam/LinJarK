...
cat prompts | sed -e 's/[^A-Za-z0-9._-]/!/g' | sed 's/!!ffne/öffne/g'| sed 's/_!ffne/_öffne/g' | sed 's/!FFNEN/ÖFFNEN/g' |  sed 's/!!default!!/\ "default"\ /g' | sed 's/schlie!!en/schließen/g' | sed 's/einf!!gen/einfügen/g'|sed 's/Einf!!gen/Einfügen/g' | sed 's/EINF!GEN/EINFÜGEN/g' | sed 's/l!!schen/löschen/g'| sed 's/L!SCHEN/LÖSCHEN/g' | sed 's/N!!chste/Nächste/g'| sed 's/N!CHSTE/NÄCHSTE/g' | sed 's/gr!!!!er/größer/g' | sed 's/GR!SSER/GRÖSSER/g' | sed 's/l!!uft/läuft/g' | sed 's/L!UFT/LÄUFT/g' | sed 's/_f!!r_/_für_/g' | sed 's/F!R/FÜR/g' | sed 's/_!!bermorgen/_übermorgen/g' | sed 's/!BERMORGEN/ÜBERMORGEN/g' | sed 's/zuf!!llig/zufällig/g' | sed 's/ZUF!LLIG/ZUÄLLIG/g' | sed 's/n!!chste/nächste/g' | sed 's/R!!ckg!!ngig/Rückgängig/g' | sed 's/R!CKG!NGIG/RÜCKGÄNGIG/g' | sed 's/Zur!!ck/Zurück/g' | sed 's/ZUR!CK/ZURÜCK/g' | sed 's/f!!nf/fünf/g' | sed 's/F!NF/FÜNF/g' | sed 's/Zw!!lf/Zwölf/g' | sed 's/ZW!LF/ZWÖLF/g' | sed 's/F!!nf/Fünf/g' | sed 's/Empf!!nger/Empfänger/g' | sed 's/EMPF!NGER/EMPFÄNGER/g' | sed 's/Ausw!!hlen/Auswählen/g' | sed 's/AUSW!HLEN/AUSWÄHLEN/g' | sed 's/!!bersetzung/Übersetzung/g' | sed 's/!BERSETZUNG!/ÜBERSETZUNG!/g' | sed 's/!!bersetze/übersetze/g' | sed 's/!BERSETZE/ÜBERSETZE/g' | sed 's/Franz!!sisch/Französisch/g' | sed 's/FRANZ!SISCH/FRANZÖSISCH/g' | sed 's/Windverh!!ltnisse/Windverhältnisse/g' | sed 's/WINDVERH!LTNISSE/WINDVERHÄLTNISSE/g' | sed 's/Windst!!rke/Windstärke/g' | sed 's/WINDST!RKE/WINDSTÄRKE/g' | sed 's/Abk!!rzung/Abkürzung/g' | sed 's/ABK!RZUNG/ABKÜRZUNG/g' | sed 's/Arbeitsfl!!che/Arbeitsfläche/g' | sed 's/ARBEITSFL!CHE/ARBEITSFLÄCHE/g' > b
